        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height//2 - 2, 10, 5)    # (comes from middle, pixel, height)
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()

                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(red.x, red.y + red.height//2 - 2, 10, 5)    # (comes from middle, pixel, height)
...
#    pygame.quit()
    """
    if you want game to loop after a game ends:
    call "main()" at the end of while loop &
    pygame.quit() in "if condition of pygame.QUIT"

    if you want game to quit after a game ends:
    just call "pygame.quit()" at the end of while loop
    (no need for pygame.quit() like the above)
    """
